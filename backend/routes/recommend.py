...
def similar_movies(douban_id):
    ...
    try:
        ...
        logger.error(f"Similar movies query failed: {e}")
        return jsonify({'error': 'Database query failed', 'movies': [], 'source': 'similar_genre'}), 503
...
def collaboration_recommend(douban_id):
    ...
    try:
        ...
        logger.error(f"Collaboration recommend query failed: {e}")
        return jsonify({'error': 'Database query failed', 'movies': [], 'source': 'collaboration'}), 503
...
def popular_movies():
    ...
    try:
        ...
        logger.error(f"Popular movies query failed: {e}")
        return jsonify({'error': 'Database query failed', 'movies': []}), 503
